...
const getMyNotifications = (req, res) => {
    notificationModel.getNotificationsByUser(req.user.id, (err, result) => {
...
const deleteNotification = (req, res) => {
    db.query("DELETE FROM notifications WHERE id=? AND utilisateur_id=?", [req.params.id, req.user.id], (err, result) => {
        if (err) return res.status(500).json(err);
